...
const copy = {
  ko: {
    ...
    dom: '같은 DOM, 서로 다른 검사',
    ui: ['UI 검사', '23종 점수 규칙 + 10종 참고', '가중치 점수 ≠ AI 생성 확률'],
    ...
    risks: [
      ['요약에서 prose 누락', '문체는 full: true로 조회'],
      ['리포트는 메모리 스냅샷', '재시작 시 소실 · 연결 해제 시 오래될 수 있음'],
      ['6자리 HEX 기본 토큰', '요청 제한도 없어 인증 보강 필요'],
    ],
    nextTitle: '04 / 다음 세 가지',
    next: ['1  MCP 인증 길이·연결 제한 강화', '2  문체 요약 포함 + 낡은 리포트 표시', '3  실제 연결·단축키 + 장르별 오탐 검증'],
    planned: ['계획만 있음 · LLM 정밀 분석 / OAuth', '현재 실행 경로와 연결되지 않음'],
    proof: ['관찰: 문체 47 / UI 정밀 19 검사 통과', 'MCP 모의 왕복 확인 · 실제 에이전트 연결은 미검증'],
    footer: '소스 7e0f0a5 · 확인 2026-09-06 · 실선: 코드 경로 / 점선: 계획',
  },
  en: {
    ...
    ui: ['UI checks', '23 scored rules + 10 references', 'Pattern score ≠ AI probability'],
    ...
    risks: [
      ['Summary omits prose', 'Use full: true for writing results'],
    ...
    next: ['1  Harden MCP tokens and connection limits', '2  Include prose summaries; mark stale reports', '3  Test live setup, shortcuts & genre false positives'],
    planned: ['Planned only · LLM review / OAuth', 'Not connected to the current execution path'],
    proof: ['Observed: 47 prose / 19 UI precision checks passed', 'Mock MCP round trips; live agent setup not verified'],
    footer: 'Source 7e0f0a5 · 2026-09-06 · Solid: code / dashed: planned',
  },
};
