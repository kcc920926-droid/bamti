...
function initTheme(): void {
  ...
  const saved = (localStorage.getItem('ledger.theme') as Theme | null) ?? 'auto';
  ...
  $$('[data-theme-set]').forEach((btn) => {
    btn.addEventListener('click', () => apply((btn.dataset.themeSet as Theme) ?? 'auto'));
  });
}
...
const PARTS: PartEntry[] = $$('[data-part]').map((el) => {
...
function initFilter(): void {
  const input = $<HTMLInputElement>('#partFilter');
  const empty = $('[data-filter-empty]');
  const count = $('[data-part-count]');
  if (count) count.textContent = String(PARTS.length) + '종';
  if (!input) return;
  ...
  const run = (): void => {
    ...
    if (empty) empty.dataset.on = String(q !== '' && shown === 0);
    if (count) count.textContent = q === '' ? String(PARTS.length) + '종' : shown + ' / ' + PARTS.length;
    if (q !== '') announce(shown + '개 부품이 일치합니다.');
  };
...
function initScrollspy(): void {
  const links = $$<HTMLAnchorElement>('.index-link[href^="#ch-"]');
...
function flashIcon(btn: HTMLElement): void {
  const svg = $('svg use', btn) as SVGUseElement | null;
  if (!svg) return;
...
/* ── 알림 조각 ───────────────────────────────────────── */
type Tone = 'success' | 'danger' | 'info' | 'undo';
...
function toast(message: string, tone: Tone = 'info'): void {
...
function initToastTriggers(): void {
  ...
  $$('[data-dismiss]').forEach((btn) => {
    btn.addEventListener('click', () => {
      const box = btn.closest<HTMLElement>('.alert, .banner');
      if (!box) return;
...
function initPalette(): void {
  ...
  if (!scrim || !input || !results) return;

  let index = 0;
  let matches: PartEntry[] = [];
  ...
  document.addEventListener('keydown', (e) => {
    if ((e.metaKey || e.ctrlKey) && e.key.toLowerCase() === 'k') {
      e.preventDefault();
      if (scrim.hidden) open(); else close();
  ...
  input.addEventListener('keydown', (e) => {
    if (e.key === 'ArrowDown') { e.preventDefault(); index = clamp(index + 1, 0, matches.length - 1); highlight(); }
    else if (e.key === 'ArrowUp') { e.preventDefault(); index = clamp(index - 1, 0, matches.length - 1); highlight(); }
    else if (e.key === 'Enter') { e.preventDefault(); go(index); }
  });

  results.addEventListener('click', (e) => {
    const opt = (e.target as HTMLElement).closest<HTMLElement>('.palette__opt');
    if (opt) go(Number(opt.dataset.i));
  });
  ...
  if (!isMac) {
    $$('.kbd').forEach((k) => {
      ...
      else if (k.textContent === '⌘') k.textContent = 'Ctrl';
...
function initDialogs(): void {
  ...
  const openDialog = (id: string): void => {
    ...
    const auto = $<HTMLElement>('[data-autofocus]', dlg) ?? $<HTMLElement>('input, button, select, textarea', dlg);
    auto?.focus();
  };
  ...
  $$<HTMLDialogElement>('dialog').forEach((dlg) => {
    $$('[data-modal-close]', dlg).forEach((btn) =>
...
function initDropdowns(): void {
  ...
  $$('[data-dropdown]').forEach((dd) => {
    ...
    const items = (): HTMLElement[] => $$('.menu__item:not([aria-disabled="true"])', menu);
    ...
    menu.addEventListener('keydown', (e) => {
      ...
      if (e.key === 'ArrowDown') { e.preventDefault(); list[clamp(at + 1, 0, list.length - 1)]?.focus(); }
      else if (e.key === 'ArrowUp') { e.preventDefault(); list[clamp(at - 1, 0, list.length - 1)]?.focus(); }
    ...
    $$('.menu__item', menu).forEach((item) => {
      item.addEventListener('click', () => {
        if (item.dataset.menuCheck === undefined) setOpen(false);
      });
    });
  });
...
function initTips(): void {
  $$('[data-tip]').forEach((host) => {
    const show = (): void => { host.dataset.open = 'true'; };
    const hide = (): void => { host.dataset.open = 'false'; };
...
function initPopovers(): void {
  $$('[data-popover]').forEach((host) => {
    ...
    const setOpen = (open: boolean): void => {
      pop.hidden = !open;
      trigger.setAttribute('aria-expanded', String(open));
      if (open) $<HTMLElement>('input, select, button', pop)?.focus();
      else trigger.focus();
    };
    ...
    document.addEventListener('click', (e) => {
      if (!pop.hidden && !host.contains(e.target as Node)) setOpen(false);
    });
    host.addEventListener('keydown', (e) => {
      if (e.key === 'Escape' && !pop.hidden) { e.stopPropagation(); setOpen(false); }
    });
  });
}
...
function initContextMenu(): void {
  ...
  const open = (x: number, y: number): void => {
    ...
    $<HTMLElement>('.menu__item', menu)?.focus();
  };
  ...
  $$('[data-btn-group]').forEach((group) => {
    ...
    btns.forEach((b) =>
      b.addEventListener('click', () => {
        ...
        if (exclusive) btns.forEach((x) => x.setAttribute('aria-pressed', 'false'));
  ...
  $$('[data-pager]').forEach((pager) => {
    ...
    btns.forEach((b) =>
      b.addEventListener('click', () => {
        ...
        if (info) info.textContent = (page - 1) * 20 + 1 + '–' + page * 20 + ' / 476개';
      }),
    );
  });
}
...
function initSliders(): void {
  $$('[data-slider]').forEach((wrap) => {
    const input = $<HTMLInputElement>('[data-slider-input]', wrap);
  ...
  $$('[data-dual]').forEach((wrap) => {
    const lo = $<HTMLInputElement>('[data-dual-input="lo"]', wrap);
    const hi = $<HTMLInputElement>('[data-dual-input="hi"]', wrap);
    ...
    const sync = (): void => {
      ...
      if (a > b - step) {
        if (document.activeElement === lo) { a = b - step; lo.value = String(a); }
        else { b = a + step; hi.value = String(b); }
      }
      const p = (v: number): number => ((v - min) / (max - min)) * 100;
      sel.setAttribute('style', 'left:' + p(a) + '%;width:' + (p(b) - p(a)) + '%');
      if (loOut) loOut.textContent = String(a);
      if (hiOut) hiOut.textContent = String(b);
    };
  ...
  $$('[data-stepper]').forEach((wrap) => {
    ...
    const sync = (): void => {
      ...
      $$('button', wrap).forEach((b) => {
        ...
        (b as HTMLButtonElement).disabled = dir < 0 ? v <= min : v >= max;
      });
    };
...
function initRatings(): void {
  $$('[data-rating]').forEach((wrap) => {
    ...
    const paint = (n: number): void => {
      stars.forEach((s, i) => {
        if (i < n) s.dataset.on = 'true';
        else delete s.dataset.on;
        s.setAttribute('aria-checked', String(i === n - 1));
      });
      if (out) out.textContent = n + ' / ' + stars.length;
    };

    const value = (): number => Number(wrap.dataset.value ?? 0);
    ...
    stars.forEach((s, i) => {
      ...
      s.addEventListener('keydown', (e) => {
        let v = value();
        if (e.key === 'ArrowRight') v = clamp(v + 1, 0, stars.length);
        else if (e.key === 'ArrowLeft') v = clamp(v - 1, 0, stars.length);
        else return;
        e.preventDefault();
...
function initPins(): void {
  $$('[data-pin]').forEach((wrap) => {
    const cells = $$<HTMLInputElement>('input', wrap);
    const status = $('[data-pin-status]');

    const sync = (): void => {
      cells.forEach((c) => {
        if (c.value) c.dataset.filled = 'true';
        else delete c.dataset.filled;
      });
...
function initTags(): void {
  $$('[data-tags]').forEach((box) => {
    ...
    const chips = (): HTMLElement[] => $$('.tag-chip', box);

    const bind = (chip: HTMLElement): void => {
    ...
    const add = (raw: string): void => {
      ...
      if (!name) return;
      if (chips().length >= MAX) { nudge(box); toast('태그는 ' + MAX + '개까지입니다', 'danger'); return; }
      const dup = chips().some((c) => c.textContent?.replace('×', '').trim() === name);
      if (dup) { nudge(box); announce(name + '은 이미 있습니다.'); return; }
    ...
    input.addEventListener('keydown', (e) => {
      if (e.key === 'Enter' || e.key === ',') {
        ...
        if (last) { last.remove(); announce('마지막 태그를 지웠습니다.'); }
      }
    });
  ...
  $$('[data-combo]').forEach((wrap) => {
    ...
    const highlight = (): void => {
      $$('.combo__opt', list).forEach((o) =>
        o.setAttribute('aria-selected', String(Number(o.dataset.i) === index)),
      );
    };
    ...
    input.addEventListener('keydown', (e) => {
      ...
      else if (e.key === 'Enter' && index >= 0) { e.preventDefault(); choose(index); }
      else if (e.key === 'Escape') { list.hidden = true; input.setAttribute('aria-expanded', 'false'); }
    });
    list.addEventListener('click', (e) => {
      const opt = (e.target as HTMLElement).closest<HTMLElement>('.combo__opt');
...
function initDropzone(): void {
  $$('[data-dropzone]').forEach((zone) => {
    const input = $<HTMLInputElement>('[data-dropzone-input]', zone);
    const list = $('[data-file-list]');

    const kb = (n: number): string =>
      n > 1024 * 1024 ? (n / 1024 / 1024).toFixed(1) + 'MB' : Math.round(n / 1024) + 'KB';

    const addRow = (file: File): void => {
      if (!list) return;
    ...
    ['dragenter', 'dragover'].forEach((t) =>
      zone.addEventListener(t, (e) => { e.preventDefault(); zone.dataset.over = 'true'; }),
    );
    ['dragleave', 'drop'].forEach((t) =>
      zone.addEventListener(t, () => { delete zone.dataset.over; }),
    );
    zone.addEventListener('drop', (e) => {
      e.preventDefault();
      const files = (e as DragEvent).dataTransfer?.files;
      if (files) Array.from(files).forEach(addRow);
    });

    $('[data-dropzone-pick]', zone)?.addEventListener('click', () => input?.click());
    input?.addEventListener('change', () => {
      if (input.files) Array.from(input.files).forEach(addRow);
      input.value = '';
    });
  });
}
...
function initColor(): void {
  $$('[data-color]').forEach((wrap) => {
    const picker = $<HTMLInputElement>('input[type="color"]', wrap);
    const hex = $<HTMLInputElement>('[data-color-hex]', wrap);
    ...
    const paint = (value: string): void => {
      ...
      preview.setAttribute(
        'style',
        'border-color:' + value + ';background:color-mix(in srgb, ' + value + ' 14%, transparent);color:' + value,
      );
    };
...
function initCalendars(): void {
  $$('[data-calendar]').forEach((cal) => {
    ...
    let picked: string | null = '2026-03-04';
    ...
    const draw = (): void => {
      ...
      $$('[data-day]', grid).forEach((btn) => {
        btn.addEventListener('click', () => {
          ...
          draw();
          const input = $<HTMLInputElement>('[data-date-input]', cal.closest('[data-datepick]') ?? document);
          if (input && picked) input.value = picked;
          announce((picked ?? '') + ' 선택');
          const pop = cal.closest<HTMLElement>('[data-date-pop]');
    ...
    grid.addEventListener('keydown', (e) => {
      const cells = $$<HTMLButtonElement>('[data-day]', grid);
  ...
  $$('[data-datepick]').forEach((host) => {
    ...
    const setOpen = (open: boolean): void => {
      pop.hidden = !open;
      trigger.setAttribute('aria-expanded', String(open));
      if (open) $<HTMLElement>('[aria-pressed="true"], [data-day]', pop)?.focus();
    };
    ...
    host.addEventListener('keydown', (e) => {
      if (e.key === 'Escape' && !pop.hidden) { setOpen(false); trigger.focus(); }
    });
  });
}

/* ── 검색 칸 · 비밀번호 · 여러 줄 ────────────────────── */
function initTextInputs(): void {
  ...
  $$('[data-pw-toggle]').forEach((btn) => {
    btn.addEventListener('click', () => {
      const input = $<HTMLInputElement>('[data-pw]', btn.closest('.input-wrap') ?? document);
      if (!input) return;
      const show = input.type === 'password';
      input.type = show ? 'text' : 'password';
      btn.setAttribute('aria-pressed', String(show));
      btn.setAttribute('aria-label', show ? '비밀번호 가리기' : '비밀번호 보기');
      const use = $('svg use', btn) as SVGUseElement | null;
      use?.setAttribute('href', show ? '#i-lock' : '#i-eye');
    });
  });

  $$('[data-autogrow]').forEach((ta) => {
    const area = ta as unknown as HTMLTextAreaElement;
  ...
  if (meter) {
    const input = $<HTMLInputElement>('[data-pw-input]', meter);
  ...
  $$('[data-check-tree]').forEach((tree) => {
    const all = $<HTMLInputElement>('[data-check-all]', tree);
    const kids = $$<HTMLInputElement>('[data-check-child]', tree);
    if (!all) return;
  ...
  if (composer) {
    const input = $<HTMLTextAreaElement>('[data-composer-input]', composer);
    const send = $<HTMLButtonElement>('[data-composer-send]', composer);
...
function initForm(): void {
  ...
  interface Problem { label: string; message: string; el: HTMLElement }

  const validate = (el: HTMLElement): string | null => {
    const input = el as HTMLInputElement;
    const label = input.dataset.label ?? '이 항목';
    if (input.type === 'checkbox') return input.checked ? null : label + '에 동의해야 합니다';
    const v = (input.value ?? '').trim();
    if (!v) return label + '을(를) 입력하세요';
    if (input.tagName === 'SELECT') return null;
    if (input.type === 'email' && !/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(v)) return '주소 형식이 맞지 않습니다 (예: name@example.com)';
    if (input.type !== 'email' && v.length < 2) return label + '은(는) 두 글자 이상이어야 합니다';
    return null;
  };

  const paint = (el: HTMLElement, message: string | null): void => {
    const field = el.closest<HTMLElement>('.field') ?? el.closest<HTMLElement>('.check');
    if (!field) return;
    if (field.classList.contains('check')) {
      const err = $('[data-check-err]');
      if (err) err.textContent = message ?? '';
      if (err) err.setAttribute('style', message ? 'display:flex' : 'display:none');
      return;
    }
  ...
  fields.forEach((el) => {
    ...
    el.addEventListener('input', () => {
      ...
      if (field?.dataset.invalid === 'true') paint(el, validate(el));
    });
  });
  ...
  form.addEventListener('submit', (e) => {
    ...
    fields.forEach((el) => {
      ...
      paint(el, message);
      if (message) problems.push({ label: (el as HTMLInputElement).dataset.label ?? '', message, el });
    });
    ...
    if (problems.length) {
      ...
      problems[0]?.el.focus();
      if (state) state.textContent = '보내지 못함 · 오류 ' + problems.length + '건';
      announce(problems.length + '곳에 오류가 있습니다. 첫 번째 칸으로 이동했습니다.');
      return;
    }

    if (summary) summary.hidden = true;
    if (state) state.textContent = '보냈습니다 · 검토 대기';
  ...
  form.addEventListener('reset', () => {
    window.setTimeout(() => {
      fields.forEach((el) => paint(el, null));
      if (summary) summary.hidden = true;
      if (state) state.textContent = '아직 보내지 않음';
    }, 0);
  });
}

/* ── 표 ──────────────────────────────────────────────── */
interface Row { name: string; cat: string; states: number; date: string; owner: string; no: string }
...
function initTable(): void {
  ...
  const all = $<HTMLInputElement>('[data-table-all]');
  ...
  const view = (): Row[] => {
    ...
    const rows = ROWS.filter((r) =>
      q === '' || (r.name + r.cat + r.owner + r.no).toLowerCase().includes(q),
    );
  ...
  const syncBulk = (): void => {
    ...
    if (selLabel) selLabel.textContent = selected.size + '개 선택';
    if (all) {
      const shown = view();
  ...
  const draw = (): void => {
    ...
    body.innerHTML = rows
      .map(
        (r) =>
          '<tr data-no="' + r.no + '" aria-selected="' + String(selected.has(r.no)) + '">' +
    ...
    if (countLabel) countLabel.textContent = rows.length + '개 부품';
    if (rangeLabel) rangeLabel.textContent = rows.length ? '1–' + rows.length + ' / ' + rows.length : '0개';

    $$<HTMLInputElement>('[data-row-check]', body).forEach((cb) => {
  ...
  all?.addEventListener('change', () => {
    const rows = view();
    if (all.checked) rows.forEach((r) => selected.add(r.no));
    else rows.forEach((r) => selected.delete(r.no));
    draw();
  });

  const search = $<HTMLInputElement>('[data-table-search]');
...
function initSortable(): void {
  $$('[data-sortable]').forEach((list) => {
    ...
    const move = (item: HTMLElement, dir: number): void => {
    ...
    $$('.sortable__item', list).forEach((item) => {
      ...
      item.addEventListener('dragover', (e) => {
        e.preventDefault();
        if (dragged && dragged !== item) item.dataset.over = 'true';
      });
      ...
      item.addEventListener('keydown', (e) => {
        ...
        else if (e.key === 'ArrowDown') { e.preventDefault(); move(item, 1); }
      });
    });
    renumber();
  });
  ...
  $$('[data-tree]').forEach((tree) => {
    ...
    rows.forEach((row) => {
      ...
      row.addEventListener('click', () => {
        if (hasKids) setOpen(row.getAttribute('aria-expanded') !== 'true');
        else {
      ...
      row.addEventListener('keydown', (e) => {
        ...
        if (e.key === 'ArrowDown') { e.preventDefault(); rows[at + 1]?.focus(); }
        else if (e.key === 'ArrowUp') { e.preventDefault(); rows[at - 1]?.focus(); }
        else if (e.key === 'ArrowRight' && hasKids) { e.preventDefault(); setOpen(true); }
        else if (e.key === 'ArrowLeft' && hasKids) { e.preventDefault(); setOpen(false); }
      });
    });
  });
}
...
function initCharts(): void {
  const bar = $<SVGSVGElement>('[data-chart="bar"]');
  if (bar) {
  ...
  const line = $<SVGSVGElement>('[data-chart="line"]');
  if (line) {
    const values = [4, 6, 5, 9, 12, 11, 15, 18, 17, 21];
    const W = 320, H = 150, PAD_L = 24, PAD_B = 20, PAD_T = 10;
    const max = 24;
    const x = (i: number): number => PAD_L + ((W - PAD_L - 8) * i) / (values.length - 1);
    const y = (v: number): number => H - PAD_B - ((H - PAD_B - PAD_T) * v) / max;
  ...
  const spark = $<SVGSVGElement>('[data-chart="spark"]');
  if (spark) {
  ...
  const paintRing = (ring: HTMLElement, pct: number): void => {
    const circle = $('.fill', ring) as SVGCircleElement | null;
...
function initProgress(): void {
  ...
  $('[data-progress-run]')?.addEventListener('click', () => {
    ...
    const tick = window.setInterval(() => {
      ...
      if (fill) fill.setAttribute('style', 'width:' + n + '%');
  ...
  $('[data-skeleton-toggle]')?.addEventListener('click', (e) => {
    ...
    (e.currentTarget as HTMLElement).textContent = showReal ? '뼈대로 되돌리기' : '실제 내용으로 바꾸기';
  });
  ...
  $$('[data-steps]').forEach((wrap) => {
    ...
    const draw = (): void => {
      ...
      if (label2) label2.textContent = at + 1 + ' / ' + steps.length + ' 단계';
      const prev = $<HTMLButtonElement>('[data-step-prev]', wrap);
      const next = $<HTMLButtonElement>('[data-step-next]', wrap);
      if (prev) prev.disabled = at === 0;
      if (next) next.textContent = at === steps.length - 1 ? '검토 요청 보내기' : '다음';
    };

    $('[data-step-prev]', wrap)?.addEventListener('click', () => { at = clamp(at - 1, 0, steps.length - 1); draw(); });
    $('[data-step-next]', wrap)?.addEventListener('click', () => {
      if (at === steps.length - 1) { toast('검토 요청을 보냈습니다', 'success'); return; }
...
function initCarousel(): void {
  $$('[data-carousel]').forEach((car) => {
    ...
    view.addEventListener('scroll', () => {
      ...
      if (!dots) return;
      $$('button', dots).forEach((d) => {
        if (Number(d.dataset.i) === at) d.setAttribute('aria-current', 'true');
...
function initSplit(): void {
  $$('[data-split]').forEach((pane) => {
    ...
    if (saved) pane.style.setProperty('--split', saved + '%');

    const set = (pct: number): void => {
    ...
    grip.addEventListener('keydown', (e) => {
      ...
      if (e.key === 'ArrowLeft') { e.preventDefault(); set(now - 2); }
      else if (e.key === 'ArrowRight') { e.preventDefault(); set(now + 2); }
    });
  });
}
...
function initBoard(): void {
  ...
  const bindCard = (card: HTMLElement): void => {
    ...
    card.addEventListener('keydown', (e) => {
      ...
      e.preventDefault();
      const col = card.closest<HTMLElement>('[data-board-col]');
      if (!col) return;
