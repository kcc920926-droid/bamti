...
function setTheme(theme) {
    ...
    themeToggle?.setAttribute("aria-label", theme === "light" ? "다크 모드로 전환" : "라이트 모드로 전환");
}
...
const sections = [
    ...document.querySelectorAll(".catalog-section, #overview"),
];
const navLinks = [
...
updateRange();
let stepperValue = 4;
const stepperOutput = document.querySelector("#stepperOutput");
document
    .querySelectorAll("[data-step]")
    .forEach((button) => {
    button.addEventListener("click", () => {
        stepperValue = Math.max(0, Math.min(99, stepperValue + Number(button.dataset.step)));
        if (stepperOutput)
...
pinInputs.forEach((input, index) => {
    ...
    input.addEventListener("paste", (event) => {
        ...
        [...code].forEach((char, itemIndex) => {
            if (pinInputs[itemIndex])
                pinInputs[itemIndex].value = char;
        });
...
tableFilter?.addEventListener("input", () => {
    ...
    projectRows?.querySelectorAll("tr").forEach((row) => {
        row.classList.toggle("is-filtered", !row.textContent?.toLocaleLowerCase().includes(query));
    });
});
...
projectRows
    ?.querySelectorAll('input[type="checkbox"]')
    .forEach((checkbox) => {
    checkbox.addEventListener("change", () => checkbox.closest("tr")?.classList.toggle("is-selected", checkbox.checked));
});
let sortDirection = 1;
document
    ...
    button.addEventListener("click", () => {
        ...
        rows.sort((a, b) => (a.dataset[key] ?? "").localeCompare(b.dataset[key] ?? "", undefined, { numeric: true }) * sortDirection);
        sortDirection *= -1;
...
componentFilter?.addEventListener("input", () => {
    ...
    document.querySelectorAll(".specimen").forEach((card) => {
        const shouldHide = Boolean(query) &&
            card !== filterHost &&
    ...
    document
        ...
        .forEach((section) => {
        const cards = [...section.querySelectorAll(".specimen")];
        section.classList.toggle("is-search-hidden", Boolean(query) &&
            cards.every((card) => card.classList.contains("is-search-hidden")));
    });
});
// Feedback -------------------------------------------------------------------
document
    .querySelectorAll(".alert > button, .demo-toast > button")
    .forEach((button) => {
...
document
    ...
    ?.addEventListener("click", () => showToast("Saved successfully", "Your preferences are up to date."));
document
    .querySelector("#splitAction")
    ?.addEventListener("click", () => showToast("Export started", "Your report will be ready shortly."));
document
    .querySelector("#copyCode")
    ?.addEventListener("click", async (event) => {
    const button = event.currentTarget;
    try {
        await navigator.clipboard.writeText(document.querySelector(".code-card pre")?.textContent ?? "");
    }
    catch {
...
document
    ...
    trigger.addEventListener("click", (event) => {
        ...
        const target = document.querySelector(`#${trigger.dataset.popoverTrigger}`);
        if (!target)
            return;
...
document.addEventListener("click", (event) => {
    document
        .querySelectorAll(".mini-menu.is-visible, .profile-popover.is-visible")
        .forEach((menu) => {
        if (menu.contains(event.target))
            return;
        const trigger = document.querySelector(`[data-popover-trigger="${menu.id}"]`);
...
const overlayScrim = document.querySelector("#overlayScrim");
...
document
    ...
    .forEach((button) => button.addEventListener("click", () => openLayer("command")));
document
...
const commandButtons = [
    ...document.querySelectorAll("#commandResults > button"),
];
...
commandInput?.addEventListener("input", () => {
    ...
    commandButtons.forEach((button) => button.classList.toggle("is-search-hidden", !button.textContent?.toLocaleLowerCase().includes(query)));
});
