...
const setAriaHidden = (selector, hidden) => {
...
const setupCommandPalette = () => {
    ...
    const items = $$('[data-palette-action]');
...
const setupDismiss = () => {
    $$('[data-dismiss]').forEach((button) => {
        button.addEventListener('click', () => {
            const parent = button.closest('.alert, .toast-message');
            if (parent)
                parent.hidden = true;
        });
    });
...
const setupDemoControls = () => {
    ...
    $$('[data-command-inline]').forEach((input) => {
        input.addEventListener('keydown', (event) => {
...
const setupFiltersAndNavigation = () => {
    ...
    const sections = $$('[data-section], [data-category]');
