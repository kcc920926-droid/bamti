...
interface ToastOptions { kind?: ToastKind; ttl?: number; title?: string; }
interface RingOptions { size?: number; stroke?: number; caption?: string; }

/* ── 기본 유틸 ──────────────────────────────────────────────────── */
const qs = <T extends Element = AnyEl>(sel: string, root: ParentNode = document): T | null =>
  root.querySelector(sel) as T | null;

const qsa = <T extends Element = AnyEl>(sel: string, root: ParentNode = document): T[] =>
...
const uid = (): string => `k-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 7)}`;
...
function applyTheme(theme: 'light' | 'dark'): void {
  ...
  try { localStorage.setItem(THEME_KEY, theme); } catch { /* file:// 에서는 무시 */ }
}

function initTheme(): void {
  const saved = ((): 'light' | 'dark' | null => {
  ...
  qsa<HTMLButtonElement>('[data-theme-toggle]').forEach(btn => {
...
function toast(msg: string, opts: ToastOptions = {}): void {
  ...
  let region = qs<AnyEl>('.toast-region');
  if (!region) {
  ...
  const kill = (): void => {
    ...
    const rm = (): void => el.remove();
    el.addEventListener('animationend', rm, { once: true });
    setTimeout(rm, 420); // animation 미지원 (reduced-motion 등) 폴백
  };
  qs<HTMLButtonElement>('.t-close', el)?.addEventListener('click', kill);
  setTimeout(kill, ttl);
  if (qsa('.toast', region).length > 4) qsa('.toast', region)[0]?.classList.add('out');
}
...
/* ── 링 게이지 ───────────────────────────────────────────────────── */
function renderRing(el: AnyEl, value: number, opts: RingOptions = {}): { set: (v: number) => void } {
  ...
  if (!qs('svg', el)) {
    ...
    valEl = qs<AnyEl>('.rg-val', el);
  }

  const set = (v: number): void => {
    const n = clamp(v, 0, 100);
    const prog = qs<SVGCircleElement>('.rg-prog', el);
    if (prog) prog.style.strokeDashoffset = String(c * (1 - n / 100));
    el.classList.toggle('ok', n >= 70);
    el.classList.toggle('danger', n <= 30);
    const vv = valEl ?? qs<AnyEl>('.rg-val', el);
    if (vv) vv.textContent = `${Math.round(n)}%`;
  };
  set(value);
  return { set };
}
...
function initRings(): void {
  qsa<AnyEl>('[data-ring]').forEach(el => {
...
function openModal(id: string): void {
  const backdrop = qs<AnyEl>(`#${id}`);
  if (!backdrop) return;
  backdrop.setAttribute('open', '');
  document.body.style.overflow = 'hidden';
  const first = qs<AnyEl>('button, a, input, [tabindex]', backdrop);
  first?.focus();
}
...
function initOverlays(): void {
  qsa<HTMLButtonElement>('[data-modal-open]').forEach(b =>
    b.addEventListener('click', () => openModal(b.dataset.modalOpen!)));
  qsa<HTMLButtonElement>('[data-drawer-open]').forEach(b =>
    b.addEventListener('click', () => openModal(b.dataset.drawerOpen!)));
  qsa<AnyEl>('[data-modal-close]').forEach(b =>
    b.addEventListener('click', () => {
      const back = b.closest('.modal-backdrop, .drawer-backdrop') as AnyEl | null;
      if (back) closeModal(back.id);
    }));
  qsa<AnyEl>('.modal-backdrop, .drawer-backdrop').forEach(back => {
    back.addEventListener('click', e => { if (e.target === back) closeModal(back.id); });
  });
  document.addEventListener('keydown', e => {
    if (e.key !== 'Escape') return;
    qsa<AnyEl>('.modal-backdrop[open], .drawer-backdrop[open]').forEach(b => closeModal(b.id));
  });
}

/* confirm 다이얼로그 */
function confirmDialog(msg: string, title = '확인 필요'): Promise<boolean> {
  return new Promise(resolve => {
    ...
    qs<HTMLButtonElement>('[data-c-no]', back)!.addEventListener('click', () => done(false));
    qs<HTMLButtonElement>('[data-c-yes]', back)!.addEventListener('click', () => done(true));
    openModal(id);
  });
}
...
function initDropdowns(): void {
  document.addEventListener('click', (e) => {
    const trig = (e.target as AnyEl).closest<HTMLButtonElement>('[data-dropdown]');
    qsa<AnyEl>('.drop-menu[open]').forEach(m => {
      const owner = m.closest<AnyEl>('.dropdown')?.querySelector<HTMLButtonElement>('[data-dropdown]');
      if (owner !== trig) m.removeAttribute('open');
    });
    if (trig) {
      const menu = trig.parentElement!.querySelector<AnyEl>('.drop-menu');
      menu?.toggleAttribute('open');
      e.stopPropagation();
    }
  });
  document.addEventListener('keydown', e => {
    if (e.key === 'Escape') qsa<AnyEl>('.drop-menu[open]').forEach(m => m.removeAttribute('open'));
  });
  qsa<AnyEl>('.drop-menu .mi:not([aria-disabled="true"])').forEach(mi => {
...
function initTabs(): void {
  qsa<AnyEl>('[data-tabs]').forEach(group => {
    const btns = qsa<HTMLButtonElement>('.tab', group);
    btns.forEach(btn => btn.addEventListener('click', () => {
      btns.forEach(b => b.setAttribute('aria-selected', String(b === btn)));
      const target = btn.dataset.tab;
      qsa<AnyEl>('[data-panel]', group).forEach(p =>
...
function initAccordions(): void {
  qsa<HTMLButtonElement>('.acc-head').forEach(head => {
    head.addEventListener('click', () => {
      const item = head.closest<AnyEl>('.acc-item')!;
      const root = head.closest<AnyEl>('.accordion')!;
      if (root.dataset.exclusive === 'true' && !item.hasAttribute('open')) {
        qsa<AnyEl>('.acc-item[open]', root).forEach(o => o.removeAttribute('open'));
      }
...
function initSegmented(): void {
  qsa<AnyEl>('[data-seg]').forEach(seg => {
    qsa<HTMLButtonElement>('.seg-btn', seg).forEach(btn =>
      btn.addEventListener('click', () => {
        qsa<HTMLButtonElement>('.seg-btn', seg).forEach(b =>
          b.setAttribute('aria-pressed', String(b === btn)));
        const msg = seg.dataset.segMsg;
        if (msg) toast(msg.replace('{v}', btn.dataset.value ?? btn.textContent ?? ''), { kind: 'ok', title: '세그먼트' });
      }));
  });
}
...
function initSliders(): void {
  qsa<HTMLInputElement>('input[type="range"][data-bubble]').forEach(range => {
    const wrap = range.parentElement!.querySelector<AnyEl>('.range-bubble');
    if (!wrap) return;
    const paint = (): void => {
  ...
  /* 링 게이지 연동 데모 */
  const slider = qs<HTMLInputElement>('#ring-slider');
  const ring = qs<AnyEl>('#ring-demo');
  if (slider && ring) {
  ...
  /* 프로그레스 데모 */
  const progSlider = qs<HTMLInputElement>('#prog-slider');
  const progBar = qs<AnyEl>('#prog-demo .bar');
  const progPct = qs<AnyEl>('#prog-pct');
...
function initPasswordToggle(): void {
  qsa<HTMLButtonElement>('[data-pw-toggle]').forEach(btn => {
    btn.innerHTML = I_EYE_OFF;
    btn.addEventListener('click', () => {
      const inp = qs<HTMLInputElement>('#' + btn.dataset.pwToggle);
      if (!inp) return;
...
/* ── 인풋 클리어 버튼 ────────────────────────────────────────────── */
function initClearButtons(): void {
  qsa<HTMLButtonElement>('[data-clear]').forEach(btn => {
    btn.addEventListener('click', () => {
      const inp = qs<HTMLInputElement>('#' + btn.dataset.clear);
      if (inp) { inp.value = ''; inp.focus(); }
      const suf = btn.closest<AnyEl>('.suffix');
      if (suf) delete suf.dataset.show;
    });
  });
  document.addEventListener('input', e => {
    ...
    const btn = wrap?.querySelector<HTMLButtonElement>('[data-clear]');
    const suf = btn?.closest<AnyEl>('.suffix');
    if (btn && suf) {
      if (inp.value) suf.dataset.show = '1';
      else delete suf.dataset.show;
    }
  });
}
...
function initAlerts(): void {
  document.addEventListener('click', e => {
    const closeBtn = (e.target as AnyEl).closest<HTMLButtonElement>('.a-close');
    if (closeBtn) closeBtn.closest('.alert')?.remove();
  });
}

/* ── 텍스트영역 카운터 ───────────────────────────────────────────── */
function initCounters(): void {
  qsa<AnyEl>('[data-counter]').forEach(counter => {
    const txt = qs<HTMLTextAreaElement>(counter.dataset.counter!);
    if (!txt) return;
    const paint = (): void => {
...
function initOtp(): void {
  qsa<AnyEl>('[data-otp]').forEach(otp => {
    const inputs = qsa<HTMLInputElement>('.otp', otp);
    ...
    if (inputs.length > 4) {
      const btn = qs<HTMLButtonElement>('.btn', otp.parentElement!);
      btn?.addEventListener('click', () => {
...
/* ── 칩 인풋 (태그) ──────────────────────────────────────────────── */
function initChipsInputs(): void {
  qsa<AnyEl>('[data-chips]').forEach(wrap => {
    const input = qs<HTMLInputElement>('input', wrap)!;
    ...
    input.addEventListener('blur', () => {
      if (input.value) { add(input.value); input.value = ''; }
    });
...
function initComboboxes(): void {
  qsa<AnyEl>('[data-combo]').forEach(box => {
    const input = qs<HTMLInputElement>('input', box)!;
    const list = qs<AnyEl>('.combo-list', box)!;
    const items = qsa<AnyEl>('.co-item', list);
    const open = (): void => {
      list.setAttribute('open', '');
      list.querySelectorAll('.co-item').forEach(i => {
        const hay = (i.textContent ?? '').toLowerCase();
        (i as AnyEl).style.display = hay.includes(input.value.toLowerCase()) ? '' : 'none';
      });
    };
    ...
    input.addEventListener('keydown', e => {
      if (e.key === 'ArrowDown') (list.querySelector<AnyEl>('.co-item:not([style*="none"])'))?.focus();
    });
    const itemVal = (item: AnyEl): string =>
      item.dataset.value ?? (item.textContent ?? '').replace(item.querySelector('.sub')?.textContent ?? '', '').trim();
    ...
    input.addEventListener('keydown', e => {
      if (e.key === 'Enter') {
        const visible = qsa<AnyEl>('.co-item', list).find(i => i.style.display !== 'none');
        if (visible) { input.value = itemVal(visible); list.removeAttribute('open'); }
      }
    });
    document.addEventListener('click', e => {
      if (!box.contains(e.target as Node)) list.removeAttribute('open');
    });
  });
}
...
function initRatings(): void {
  qsa<AnyEl>('[data-rating]').forEach(rating => {
    const stars = qsa<HTMLButtonElement>('.star', rating);
...
function initCopy(): void {
  qsa<HTMLButtonElement>('[data-copy]').forEach(btn => {
    btn.addEventListener('click', async () => {
      const target = btn.dataset.copy!;
      const el = qs<AnyEl>(target);
...
function initTableSort(): void {
  qsa<HTMLTableElement>('[data-sort]').forEach(table => {
    qsa<HTMLTableCellElement>('th.sortable', table).forEach(th => {
      th.addEventListener('click', () => {
        const idx = Array.from(th.parentElement!.children).indexOf(th);
        const dir = th.dataset.dir === 'asc' ? 'desc' : 'asc';
        qsa<HTMLTableCellElement>('th.sortable', table).forEach(h => { delete h.dataset.dir; });
        th.dataset.dir = dir;

        const rows = Array.from(table.tBodies[0]?.rows ?? []);
        const num = th.dataset.num === 'true';
        const get = (r: HTMLTableRowElement): string | number => {
          const v = r.cells[idx]?.textContent?.trim() ?? '';
          return num ? parseFloat(v.replace(/[^0-9.-]/g, '')) || 0 : v;
        };
        rows.sort((a, b) => {
          const va = get(a), vb = get(b);
          const cmp = (num ? (va as number) - (vb as number) : String(va).localeCompare(String(vb), 'ko'));
...
/* ── 테이블 선택 (체크박스 + 툴바) ────────────────────────────────── */
function initTableSelect(): void {
  qsa<AnyEl>('[data-select-all]').forEach(all => {
    const tb = all.closest<AnyEl>('.table-wrap')!;
    all.addEventListener('change', () => {
      qsa<HTMLInputElement>('.row-check', tb).forEach(c => c.checked = (all as HTMLInputElement).checked);
      syncSelCount(tb);
    });
    qsa<HTMLInputElement>('.row-check', tb).forEach(c =>
      c.addEventListener('change', () => {
        const checks = qsa<HTMLInputElement>('.row-check', tb);
        (all as HTMLInputElement).checked = checks.every(x => x.checked);
        (all as HTMLInputElement).indeterminate = checks.some(x => x.checked) && !checks.every(x => x.checked);
        syncSelCount(tb);
      }));
  });
}
function syncSelCount(tb: AnyEl): void {
  const n = qsa<HTMLInputElement>('.row-check:checked', tb).length;
  const count = tb.querySelector<AnyEl>('.sel-count');
  const bulk = tb.querySelector<AnyEl>('.bulk-bar');
  if (!count) return;
...
function initTrees(): void {
  qsa<AnyEl>('[data-tree]').forEach(tree => {
    qsa<HTMLButtonElement>('.tr-toggle', tree).forEach(tog => {
      tog.addEventListener('click', () => {
        tog.classList.toggle('open');
        const kids = tog.closest<AnyEl>('.tr-node')?.querySelector<AnyEl>(':scope > .tr-children');
        kids?.classList.toggle('collapsed');
      });
    });
    qsa<AnyEl>('.tr-row', tree).forEach(row => {
      row.addEventListener('click', () => {
        qsa<AnyEl>('.tr-row.selected', tree).forEach(r => r.classList.remove('selected'));
...
function initPagination(): void {
  qsa<AnyEl>('[data-pagination]').forEach(pg => {
    const btn = (sel: string): HTMLButtonElement | null => qs<HTMLButtonElement>(sel, pg);
    const pages = qsa<HTMLButtonElement>('.pg-btn:not(.pg-prev):not(.pg-next)', pg);
...
function initSteppers(): void {
  qsa<AnyEl>('[data-stepper]').forEach(sp => {
    const steps = qsa<AnyEl>('.step', sp);
    const btnPrev = qs<HTMLButtonElement>('.btn', sp.parentElement!)?.matches('[data-step-prev]')
      ? qs<HTMLButtonElement>('[data-step-prev]', sp.parentElement!) : null;
    const btnNext = qs<HTMLButtonElement>('[data-step-next]', sp.parentElement!);
    ...
    const paint = (): void => {
      ...
      if (btnNext) { btnNext.disabled = cur === steps.length - 1; btnNext.textContent = cur === steps.length - 2 ? '완료' : '다음'; }
    };
    btnNext?.addEventListener('click', () => { if (cur < steps.length - 1) { cur++; paint(); }
      else toast('모든 단계 완료!', { kind: 'ok', title: '스텝퍼' }); });
    btnPrev?.addEventListener('click', () => { if (cur > 0) { cur--; paint(); } });
    paint();
  });
}
...
function initPalette(): void {
  const back = qs<AnyEl>('[data-palette]');
  if (!back) return;
  const input = qs<HTMLInputElement>('.p-input-row input', back)!;
  const list = qs<AnyEl>('.p-list', back)!;
  ...
  const close = (): void => back.removeAttribute('open');

  document.addEventListener('keydown', e => {
    if ((e.metaKey || e.ctrlKey) && e.key.toLowerCase() === 'k') { e.preventDefault(); open(); }
    if (e.key === 'Escape' && back.hasAttribute('open')) close();
  });
  back.addEventListener('click', e => { if (e.target === back) close(); });

  const run = (item: AnyEl): void => {
    const target = item.dataset.target;
    close();
    if (target) {
      qs<AnyEl>(target)?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }
  ...
  input.addEventListener('input', () => {
    ...
    qsa<AnyEl>('.p-item', list).forEach(it => {
      const hay = (it.textContent ?? '').toLowerCase();
      it.style.display = hay.includes(q) ? '' : 'none';
    });
    qsa<AnyEl>('.p-sec', list).forEach(sec => {
      const visible = qsa<AnyEl>('.p-item', sec).some(it => it.style.display !== 'none');
      sec.style.display = visible ? '' : 'none';
    });
  });

  list.addEventListener('click', e => {
    const item = (e.target as AnyEl).closest<AnyEl>('.p-item');
    if (item) run(item);
  });

  input.addEventListener('keydown', e => {
    const items = qsa<AnyEl>('.p-item', list).filter(it => it.style.display !== 'none');
  ...
  qsa<AnyEl>('.p-item', list).forEach(it => it.addEventListener('mousemove', () => {
    qsa<AnyEl>('.p-item', list).forEach(x => x.removeAttribute('aria-selected'));
...
function initDropzones(): void {
  qsa<AnyEl>('[data-dropzone]').forEach(dz => {
    const input = qs<HTMLInputElement>('input[type="file"]', dz) ?? qs<HTMLInputElement>('input[type="file"]', dz.parentElement!);
    const label = qs<AnyEl>('.dz-label', dz);
    const fake = (name: string, size: string): void => {
      if (label) {
    ...
    dz.addEventListener('drop', e => {
      ...
      if (f) fake(f.name, `${(f.size / 1024).toFixed(1)}KB`);
    });
...
function initLoadingButtons(): void {
  qsa<HTMLButtonElement>('[data-loading]').forEach(btn => {
    btn.addEventListener('click', () => {
...
/* ── 스켈레톤 토글 ───────────────────────────────────────────────── */
function initSkeletonToggle(): void {
  qsa<HTMLButtonElement>('[data-skeleton-toggle]').forEach(btn => {
    btn.addEventListener('click', () => {
      const target = qs<AnyEl>('#' + btn.dataset.skeletonToggle);
      if (!target) return;
...
function initPopovers(): void {
  qsa<HTMLButtonElement>('[data-popover-open]').forEach(trigger => {
    trigger.addEventListener('click', () => {
      const id = trigger.dataset.popoverOpen!;
      const pv = qs<AnyEl>(`#${id}`);
      if (!pv) return;
  ...
  document.addEventListener('click', e => {
    if (!(e.target as AnyEl).closest('[data-popover-open], .popover')) {
      qsa<AnyEl>('.popover[open]').forEach(p => p.removeAttribute('open'));
    }
  });
}
...
interface Window { KIT: typeof KIT; }
